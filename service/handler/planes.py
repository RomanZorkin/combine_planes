...
def warp_mean(warps: list[list[np.ndarray]], conf: ImageConf) -> None:
...
def shift_estimation(image_arr: np.ndarray, conf: ImageConf, num: int) -> tuple[np.ndarray, ...]:
    r"""Функция расчета матриц смещения.
    ...
    _, warp_matrix_r = cv2.findTransformECC(
        chunk_img_g, chunk_img_r, conf.warp.red, conf.warp.mode,
    )
    _, warp_matrix_b = cv2.findTransformECC(
        chunk_img_g, chunk_img_b, conf.warp.blue, conf.warp.mode,
    )
...
def make_warps(image: np.ndarray, conf: ImageConf) -> None:
    ...
    warps: list[list[Any]] = [[], []]
    for num, _ in enumerate(conf.box):
...
def convert(image: np.ndarray, conf: ImageConf) -> np.ndarray:
    ...
    new_img[:, :, conf.channels.red] = cv2.warpAffine(  # noqa:WPS317
        image[:, :, conf.channels.red],
        conf.warp.red,
        (width, height),
        flags=cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP,
    )
    new_img[:, :, conf.channels.blue] = cv2.warpAffine(  # noqa:WPS317
